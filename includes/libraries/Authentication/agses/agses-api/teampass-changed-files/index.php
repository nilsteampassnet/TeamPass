...
/*} elseif ((!isset($_SESSION['validite_pw']) || empty($_SESSION['validite_pw']) || empty($_SESSION['user_id'])) && !empty($_GET['page'])) {
	...
	if (isset($_SESSION['settings']['agses_sm_enabled']) && $_SESSION['settings']['agses_sm_enabled'] == 1) {			
		// Agses Flicker/Response Field
		echo '<div id="connect_agses_confirm" style="margin-bottom:3px;display:none;">
		
			<img id="agses_flickering" src="axs/axs/code2gif.php?code=" width="460" height="120" alt="AGSES Flicker Code" title="Animated GIF Flicker Code"><br/>
			<label for="agses_response_code" class="form_label">Response Code</label>
			<input type="text" size="10" id="agses_response_code" name="agses_response_code" onkeypress="if (event.keyCode == 13) identifyUser(\''.$nextUrl.'\', \'', '', '\')" class="input_text text ui-widget-content ui-corner-all" />	
