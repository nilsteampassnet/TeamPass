 ...
 * @version       2.1.23
 * @copyright     (c) 2009-2015 Nils Laumaill̩
...
// User's language loading
    ...
    if ($proceedIdentification === true) {
		...
		if (isset($_SESSION['settings']['agses_sm_enabled']) && $_SESSION['settings']['agses_sm_enabled'] == 1 && $userPasswordVerified) {
			
			//check for agses-card number			
			$agses_apn = $data['agsesapn'];
			
			if ($agses_apn != "" && strlen($agses_apn) == 12 ) {				
				
				// Create Integration link
				// Include class file for web service client 				
				
				if(isset($_SESSION['settings']['agses_sm_type']) && $_SESSION['settings']['agses_sm_type'] == "ownsm") {
					
					if(!class_exists("AXSILService_V1_Auth")) {
						require_once('../axs/axs/include/axs_il_service_v1_auth.inc.php');
					}	
					// Create and initialize the web service client	
					$axsILService = new AXSILService_V1_Auth();
					
					// Set Credentials from AdminSettins
					$axsILService->setUrl($_SESSION['settings']['agses_sm_url']);
					$axsILService->setUsername($_SESSION['settings']['agses_sm_username']);
					$axsILService->setPassword($_SESSION['settings']['agses_sm_pwd']);
					
				} else {
					
					if(!class_exists("AXSILPortal_V1_Auth")) {
						require_once('../axs/axs/include/axs_il_portal_v1_auth.inc.php');
					}	
					// Create and initialize the web service client	
					$axsILService = new AXSILPortal_V1_Auth();
					
					// Set Credentials from AdminSettins
					$axsILService->setUrl($_SESSION['settings']['agses_aahosted_url']);
					$axsILService->setAAId($_SESSION['settings']['agses_aahosted_aaid']);
					$axsILService->setApiKey($_SESSION['settings']['agses_aahosted_apikey']);
				...
				//if user has card number stored, return to login for one time password
				if (!isset($dataReceived['agses_response_code']))
				{
					if (!isset($_SESSION['hedgeId']) || $_SESSION['hedgeId'] == "") {
						$_SESSION['hedgeId'] = md5(time());
					}	
	
					$_SESSION['flickercode'] = $axsILService->createAuthenticationMessage($agses_apn, true, 1, 2, $_SESSION['hedgeId']);			
					
					$return = $_SESSION['flickercode'];
					$logError = "agses_required";
				...
				//verify agses response code
				if (isset($dataReceived['agses_response_code']))
				{
					$responseCode = htmlspecialchars_decode($dataReceived['agses_response_code']);		
					
					if ($responseCode != "" && strlen($responseCode) >= 4) {					
					
						// Verify response code, store result in session
						$result = $axsILService->verifyResponse($agses_apn, $responseCode, $_SESSION['hedgeId']);
