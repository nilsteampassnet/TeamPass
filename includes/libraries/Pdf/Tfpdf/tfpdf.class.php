...
class TFPDF
{
    ...
    *                                                                              *
    *******************************************************************************/
    public function tFPDF($orientation = 'P', $unit = 'mm', $format = 'A4')
    {
        //Some checks
        $this->_dochecks();
    ...
    public function AddFont($family, $style = '', $file = '', $uni = false)
    {
        ...
        if ($uni) {
            if (defined("_SYSTEM_TTFONTS") && file_exists(_SYSTEM_TTFONTS.$file)) {
                $ttfilename = _SYSTEM_TTFONTS.$file ;
            } else {
                $ttfilename = $this->_getfontpath().'unifont/'.$file ;
            }
            $filename = $file;
        ...
        if ($file) {
            ...
            } elseif ($uni && $type  == "TrueTypesubset") {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($h <= 0x7F) {
                ...
                if (($h <= 0xDF) && ($i < $len -1)) {
                     ...
                     $out[] = ($h & 0x0F) << 12 | (ord($str{++$i}) & 0x3F) << 6 | (ord($str{++$i}) & 0x3F);
