...
global $LANG;
$LANG = array (
    'user_ga_code' => 'Send GoogleAuthenticator flash code to user by email',
    ...
    'settings_attachments_encryption_tip' => 'THIS OPTION COULD BREAK EXISTING ATTACHMENTS, please read carefully the next. If enabled, Items attachments are stored encrypted on the server. The encryption uses the SALT defined for Teampass. It requires more server ressources. WARNING: once you change the strategy, you have to run the script to adapt existing attachments. See tab \'Specific Actions\'.',
    ...
    'admin_action_pw_prefix_correct_tip' => 'Before launching this script, PLEASE be sure to make a dump of the database. This script will perform an update of passwords prefix. It SHALL only be used if you noticed that passwords are displayed with strange prefix.',
    ...
    'settings_get_tp_info' => 'Automatically load informations about Teampass',
    'settings_get_tp_info_tip' => 'This option permits the administration page to load informations such as version and libraries usage from Teampass server.',
    'at_field' => 'Field',
    ...
    'offline_mode_warning' => 'Off-line mode permits you to export your Items into an HTML file, so that you can access them when not connected to a Teampass server. The passwords are encrypted by a Key you have given.',
    ...
    'rename' => 'Rename',
    'settings_item_extra_fields' => 'Allow Items to be completed with more Fields (by Categories)',
    'settings_item_extra_fields_tip' => 'This feature enhances the Item definition with extra fields the administrator can define and organize by Categories. All data are encrypted. Notice that this feature consumes more SQL queries (around 5 more per Field during an Item update) and may require more time for actions to be performed. This is server dependant.',
    'html' => 'html',
    'more' => 'More',
    ...
    'settings_tree_counters_tip' => 'It will display for each folder 3 counters: number of items in folder; number of items in all subfolders; number of subfolders. This feature needs more SQL queries and may require more time to display the Tree.',
