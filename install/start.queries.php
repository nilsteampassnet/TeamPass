    ...
    switch ($_POST['type']) {
        ...
        case "step_3":
            ...
            if ($dbTmp = mysqli_connect($data['db_host'], $data['db_login'], $data['db_pw'], $data['db_bdd'], $data['db_port'])) {
                ...
                echo '[{"error" : "'.addslashes(mysqli_connect_error()).'", "result" : "Failed", "multiple" : ""}]';
            }
        break;
        ...
        case "step_4":
            ...
            if (is_dir($data['sk_path'])) {
                if (is_writable($data['sk_path'])) {
                    ...
                    $dbTmp = @mysqli_connect($data['db_host'], $data['db_login'], $data['db_pw'], $data['db_port']);
        ...
        case "step_5":
            ...
            // launch
            if ($dbTmp = @mysqli_connect($data['db_host'], $data['db_login'], $data['db_pw'], $data['db_port'])) {
                if (@mysqli_select_db($dbTmp, $_SESSION['db_bdd'])) {
                    $mysqli_result = "";
                    ...
                    echo '[{"error" : "Failed to connect to table", "result" : "Failed", "multiple" : ""}]';
                }
            } else {
                echo '[{"error" : "Failed to connect to server", "result" : "Failed", "multiple" : ""}]';
            }
