...
**Rule: never reimplement this transition — call `movePersonalItemToSharedFolderSynchronously()`**
(`sources/main.functions.php`). It is the single implementation shared by the web `move_item`,
the web `mass_move_items` and the API `PUT /item/update`.

```
Phase 1 — read-only, OUTSIDE the transaction (decryption is the slow part):
  1. load item + resolve source/target personal flags via getFolderIdentityWithPersonalFlag()
  2. load the caller public key
  3. decryptUserObjectKeyWithMigration() on sharekeys_items / _fields / _files
     → any missing or undecryptable key throws; nothing has been written yet

Phase 2 — transaction:
  4. SELECT ... FOR UPDATE on the items row alone (no join: a join would also lock the folder)
  5. revalidate id_tree + updated_at against phase 1 → RuntimeException on concurrent change
  6. storeUsersShareKey() ×3 families (personal=0, deleteAll=true, apiUserId=caller)
  7. UPDATE items SET id_tree, perso=0, updated_at
  8. COMMIT
```

**Rule: the fan-out stays inside the transaction.** Writing foreign sharekeys before the folder
change is committed would leave them on an item that is still personal if anything fails —
invariant I1 (SEC-8). The rollback is what guarantees that never happens.

**Rule: a missing source key aborts the move — it never deletes data.** The pre-3.2.2 web path
deleted the `categories_items` row when the moving user had no field sharekey; that is gone.
The item stays personal and the user is told to run the encryption-keys repair task
(`error_move_missing_sharekey`).

**Personal-flag resolution:** `getFolderIdentityWithPersonalFlag()` mirrors
`ItemModel::getFolderSettings()` — a folder counts as personal when its own `personal_folder`
column is set **or** when it sits inside a personal root (`nleft`/`nright` containment). Both
sides must use it, otherwise a legacy personal subfolder whose own flag was never written makes
the caller enter the branch and the callee reject the move.

**Exception contract** (`UnexpectedValueException` extends `RuntimeException` — order the
catches accordingly):

| Exception | Meaning | Web | API |
|---|---|---|---|
| `UnexpectedValueException` | source key missing/unusable | `error_move_missing_sharekey` | `422` |
| `RuntimeException` | item changed during preparation | `error_item_currently_being_updated` | `409` |
| `InvalidArgumentException` | not a personal→shared move, item/folder missing | `error_unknown` | `422` |

**Rule: the helper performs no authorization check.** Callers own it (web:
`getCurrentAccessRights()` + `user-accessible_folders`; API: `FolderAccessModel`).

**Post-commit effects** — `finalizeItemMoveSideEffects()`: `at_moved` log, item cache refresh,
source/destination folder counters, `item_moved` WebSocket event to both folders. The cache and
counter steps are switchable (`$refreshCache`, `$adjustCounters`) so `mass_move_items` can batch
them once for the whole selection.

**Not covered:** `sharekeys_logs` are intentionally not redistributed (password history is
encrypted with the application master key, so history display is unaffected).

**Orphaned sharekey invariant:** a `categories_items` row with `encryption_type = TP_ENCRYPTION_NAME` must always have at least one corresponding row in `sharekeys_fields`. If violated, the encrypted value cannot be recovered — the move is refused so an operator can repair the keys.
