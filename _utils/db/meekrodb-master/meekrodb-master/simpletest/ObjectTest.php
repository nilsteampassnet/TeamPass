...
class ObjectTest extends SimpleTest {
    ...
    function test_3_more_inserts() {
        ...
        $this->mdb->param_char = '###';
        $bart = $this->mdb->queryFirstRow ( "SELECT * FROM accounts WHERE age IN ###li AND height IN ###ld AND username IN ###ls", array (
                15,
                25
        ), array (
                10.371,
        ...
        $charlie_password = $this->mdb->queryFirstField ( "SELECT password FROM accounts WHERE username IN %ls AND username = %s", array (
                'Charlie',
                'Charlie\'s Friend'
        ), 'Charlie\'s Friend' );
        $this->assert ( $charlie_password === 'goodbye' );

        $charlie_password = $this->mdb->queryOneField ( 'password', "SELECT * FROM accounts WHERE username IN %ls AND username = %s", array (
                'Charlie',
                'Charlie\'s Friend'
        ), 'Charlie\'s Friend' );
        $this->assert ( $charlie_password === 'goodbye' );
    ...
    function test_7_insert_update() {
        ...
        $this->mdb->insertUpdate ( 'accounts', $multiples, array (
                'age' => 914
        ) );
