...
class MeekroDB {
    ...
    public function insertOrReplace($which, $table, $datas, $options = array()) {
        ...
        if (isset ( $options ['update'] ) && is_array ( $options ['update'] ) && $options ['update'] && strtolower ( $which ) == 'insert') {
            if (array_values ( $options ['update'] ) !== $options ['update']) {
                ...
                $query_param = array (
                        "INSERT INTO %b %lb VALUES %? ON DUPLICATE KEY UPDATE $update_str",
                        $table,
                        $keys,
                        $values
                );
    ...
    public function sqleval() {
        $args = func_get_args ();
        $text = call_user_func_array ( array (
                $this,
                'parseQueryParams'
        ), $args );
    ...
    protected function preparseQueryParams() {
        ...
        $types = array (
                ...
                $this->param_char . 'ss'
        ) // search string (like string, surrounded with %'s)
;
        ...
        foreach ( $posList as $pos => $type ) {
            ...
            if (is_object ( $arg ) && ($arg instanceof WhereClause)) {
                ...
                $preparsed_sql = call_user_func_array ( array (
                        $this,
                        'preparseQueryParams'
                ), $clause_args );
    ...
    protected function parseQueryParams() {
        $args = func_get_args ();
        $chunkyQuery = call_user_func_array ( array (
                $this,
                'preparseQueryParams'
        ), $args );
        ...
        foreach ( $chunkyQuery as $chunk ) {
            ...
            if ($type == 's')
                ...
                $result = $this->escape ( "%" . str_replace ( array (
                        '%',
                        '_'
                ), array (
                        '\%',
    ...
    public function queryFirstField() {
        $args = func_get_args ();
        $row = call_user_func_array ( array (
                $this,
                'queryFirstList'
        ), $args );
        if ($row == null)
