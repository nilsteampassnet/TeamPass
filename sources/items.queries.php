            ...
            if ( count( $data_received ) > 0 )
            {
                        ...
                        )
                {
                    ...
                    if ( !empty( $_POST['diffusion'] ) )
                    {
                        foreach ( explode( ';', $data_received['diffusion'] ) as $email_address )
                        {
                            @SendEmail( $txt['email_subject_item_updated'],
...
*/
                    ...
                    )
            {
                ...
                if ( $data_item['id_user'] == $_SESSION['user_id'] || $_SESSION['is_admin'] == 1 ||
                    ( $_SESSION['user_gestionnaire'] == 1 && $_SESSION['settings']['manager_edit'] == 1 ) || $data_item['anyone_can_modify'] == 1 ||
...
*/
                ...
                if ( isset( $_SESSION['settings']['enable_delete_after_consultation'] ) && $_SESSION['settings']['enable_delete_after_consultation'] == 1 )
                {
                    if ( $data_delete['del_enabled'] == 1 || $arrData['id_user'] != $_SESSION['user_id'] )
                    {
                        ...
                        else if ( $data_delete['del_type'] == 1 && $data_delete['del_value'] <= 1 || $data_delete['del_type'] == 2 && $data_delete['del_value'] < mktime( date( 'H' ), date( 'i' ), date( 's' ), date( 'm' ), date( 'd' ), date( 'y' ) )
                                )
                        {
                            $arrData['show_details'] = 0;
                            // delete item
            ...
            if ( $data_count[0] > 0 && empty( $show_error ) )
            {
                ...
                foreach ( $rows as $reccord )
                {
                    ...
                    if ( empty( $id_managed ) || $id_managed != $reccord['id'] )
                    {
                        ...
                        if ( isset( $user_is_included_in_role ) && $user_is_included_in_role == 0 && isset( $item_is_restricted_to_role ) && $item_is_restricted_to_role == 1 && !in_array( $_SESSION['user_id'], $restricted_users_array )
                                )
                        {
                        	...
                        	$returned_data[$i]['display'] = '';*/
                                    ...
                                    if ( $reccord['perso'] == 1 || ( !empty( $reccord['restricted_to'] ) && !in_array( $_SESSION['user_id'], $restricted_users_array ) ) || ( isset( $user_is_included_in_role ) && isset( $item_is_restricted_to_role ) && $user_is_included_in_role == 0 && $item_is_restricted_to_role == 1 )
                                            )
                                    {
        ...
        case "send_email":
            ...
            else
            {
                ...
                if ( $_POST['cat'] == "request_access_to_author" )
                {
                    ...
                    $ret = @SendEmail( $txt['email_request_access_subject'],
                ...
                else if ( $_POST['cat'] == "share_this_item" )
                {
                    ...
                    $ret = @SendEmail( $txt['email_share_item_subject'],
                        str_replace(
        ...
        case "history_entry_add":
            ...
            else
            {
                        ...
                        )
                {
                    ...
                    // Prepare new line
                    $data = $db->query_first( "SELECT * FROM " . $pre . "log_items WHERE id_item = '" . $data_received['item_id'] . "' ORDER BY date DESC" );
