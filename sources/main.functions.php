...
{
    // Build $iv and $iv_base64.  We use a block size of 256 bits (AES compliant) and CBC mode.  (Note: ECB mode is inadequate as IV is not used.)
    ...
    $encrypted = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, SALT, $decrypted . md5($decrypted), MCRYPT_MODE_CBC, $iv));
    // We're done!
...
function decrypt($encrypted)
{
    ...
    $decrypted = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, SALT, base64_decode($encrypted), MCRYPT_MODE_CBC, $iv), "\0\4");
