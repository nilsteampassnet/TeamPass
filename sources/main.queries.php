...
switch ( $_POST['type'] )
{
    case "change_pw":
        ...
        else if ( isset( $_POST['change_pw_origine'] ) && $_POST['change_pw_origine'] == "admin_change" )
        {
            // Check KEY
        ...
        // ADMIN first login
        else if ( isset( $_POST['change_pw_origine'] ) && $_POST['change_pw_origine'] == "first_change" )
        {
            // update DB
            $db->query_update(
                "users",
    ...
    case "identify_user":
        ...
        else if ( mysql_num_rows( $row ) == 0 && $ldap_connection == true )
        {
    ...
    case "generate_new_password":
        ...
        if ( $_POST['key'] == $data[0] )
        {
            ...
            // send to user
            $ret = json_decode( @SendEmail( $txt['forgot_pw_email_subject_confirm'],
    ...
    case "send_wainting_emails":
        if ( isset( $_SESSION['settings']['enable_send_email_on_user_login'] ) && $_SESSION['settings']['enable_send_email_on_user_login'] == 1 && isset( $_SESSION['key'] ) )
        {
            ...
            if ( ( mktime( date( 'h' ), date( 'm' ), date( 's' ), date( 'm' ), date( 'd' ), date( 'y' ) ) - $row['valeur'] ) >= 300 || $row['valeur'] == 0 )
            {
                ...
                {
                    // send email
                    $ret = json_decode( @SendEmail( $reccord['subject'],
                            $reccord['body'],
                            $reccord['receivers']
                            ) );
