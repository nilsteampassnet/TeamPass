...
if (empty($sessionTreeStructure) === true
    ...
    } elseif (isset($_SESSION['user']['treeloadstrategy']) === true
        && $_SESSION['user']['treeloadstrategy'] === 'sequential'
    ) {
        buildNodeTree(
            0,
    ...
    if (in_array($completTree[$nodeId]->id, $session_forbiden_pfs) === false
        ...
        if ($displayThisNode === true) {
            ...
            if (in_array($completTree[$nodeId]->id, $session_groupes_visibles) === true) {
                ...
                $text .= ' (<span class=\'items_count\' id=\'itcount_' . $completTree[$nodeId]->id . '\'>' . count($session_list_restricted_folders_for_items[$completTree[$nodeId]->id]) . '</span>)';
            } else {
                $restricted = '1';
                $folderClass = 'folder_not_droppable';
