    ...
    switch ($post_type) {
            ...
            if ((isset($SETTINGS['duplicate_item']) && $SETTINGS['duplicate_item'] === '0' && $itemExists === 0)
                ...
                if (isset($SETTINGS['anyone_can_modify']) && $SETTINGS['anyone_can_modify'] === '1') {
                    if ($dataReceived['anyone_can_modify'] === '1') {
                        $itemCollab = '<i class="fa fa-pencil fa-sm mi-grey-1 pointer tip" title="'.langHdl('item_menu_collab_enable').'" ondblclick="AfficherDetailsItem(\''.$newID.'\', \'0\', \'\', \'\', \'\', true, \'\')"></i>&nbsp;&nbsp;';
                    }
                }
                ...
                if (isset($SETTINGS['copy_to_clipboard_small_icons']) && $SETTINGS['copy_to_clipboard_small_icons'] === '1') {
                    ...
                    if (empty($dataReceived['login']) === false) {
                        $itemLogin = '<span id="iconlogin_'.$newID.'" class="copy_clipboard tip" title="'.langHdl('item_menu_copy_login').'"><i class="fa fa-sm fa-user mi-black"></i>&nbsp;</span>';
                    }
                    if (empty($dataReceived['pw']) === false) {
                        $itemPw = '<span id="iconpw_'.$newID.'" class="copy_clipboard tip" title="'.langHdl('item_menu_copy_login').'"><i class="fa fa-sm fa-lock mi-black"></i>&nbsp;</span>';
                    }
        ...
        case "update_item":
            ...
            if (count($dataReceived) > 0) {
                ...
                if ((
                    ...
                    DB::delete(prefixTable('tags'), "item_id = %i", $dataReceived['id']);
                    ...
                    foreach ($tags as $tag) {
                        if (empty($tag) === false) {
                            ...
                            // prepare display
                            if (empty($tags)) {
                                $return_tags = "<span class='round-grey pointer tip' title='".langHdl('list_items_with_tag')."' onclick='searchItemsWithTags(\"".strtolower($tag)."\")'><i class='fa fa-tag fa-sm'></i>&nbsp;<span class=\"item_tag\">".strtolower($tag)."</span></span>";
                            } else {
                                $return_tags .= "&nbsp;&nbsp;<span class='round-grey pointer tip' title='".langHdl('list_items_with_tag')."' onclick='searchItemsWithTags(\"".strtolower($tag)."\")'><i class='fa fa-tag fa-sm'></i>&nbsp;<span class=\"item_tag\">".strtolower($tag)."</span></span>";
                            }
                        }
                    ...
                    if (isset($SETTINGS['enable_delete_after_consultation']) && $SETTINGS['enable_delete_after_consultation'] === '1') {
                        ...
                        DB::query("SELECT * FROM ".prefixTable("automatic_del")." WHERE item_id = %i", $dataReceived['id']);
                        if (DB::count() === 0) {
                            ...
                            if (empty($dataReceived['to_be_deleted']) === false || ($dataReceived['to_be_deleted'] > 0 && is_numeric($dataReceived['to_be_deleted']))) {
                                ...
                                DB::delete(prefixTable('automatic_del'), "item_id = %i", $dataReceived['id']);
                            }
                            // update LOG
                            logItems(
                                $dataReceived['id'],
                    ...
                    if (isset($dataReceived['restricted_to_roles']) && $SETTINGS['restricted_to_roles'] === '1') {
                        ...
                        DB::delete(prefixTable("restriction_to_roles"), "item_id = %i", $dataReceived['id']);
                        // add roles for item
                        foreach (array_filter(explode(';', $dataReceived['restricted_to_roles'])) as $role) {
                            ...
                            $dataTmp = DB::queryfirstrow("SELECT title FROM ".prefixTable("roles_title")." WHERE id= ".$role);
                            if (empty($listOfRestricted)) {
                    ...
                    if ($data['id_tree'] != $dataReceived['categorie']) {
                        // Get name of folders
                        $dataTmp = DB::query("SELECT title FROM ".prefixTable("nested_tree")." WHERE id IN %li", array($data['id_tree'], $dataReceived['categorie']));
                    ...
                    $files = $filesEdit = "";
                    // launch query
                    $rows = DB::query("SELECT id, name, file, extension FROM ".prefixTable("files")." WHERE id_item=%i", $dataReceived['id']);
                    foreach ($rows as $record) {
                        ...
                        if (in_array($record['extension'], TP_IMAGES_FILE_EXT)) {
                        ...
                        foreach (explode(';', $dataReceived['diffusion']) as $emailAddress) {
                            if (empty($emailAddress) === false) {
                                sendEmail(
                                    ...
                                    $LANG,
                                    $SETTINGS,
                                    str_replace("#item_label#", $label, langHdl('email_bodyalt_item_updated'))
                                );
                            }
                        }
                    }
        ...
        */
        case "copy_item":
            // Check KEY and rights
            if ($post_key !== $_SESSION['key']) {
                $returnValues = '[{"error" : "not_allowed"}, {"error_text" : "1'.langHdl('error_not_allowed_to').'"}]';
                echo $returnValues;
                break;
            }
            ...
            if (empty($post_item_id) === false
                ...
                // previous is public folder and personal one
                    ...
                    if (!isset($_SESSION['user_settings']['session_psk']) || empty($_SESSION['user_settings']['session_psk'])) {
                        $returnValues = '[{"error" : "no_psk"}, {"error_text" : "'.langHdl('alert_message_personal_sk_missing').'"}]';
                        echo $returnValues;
                        break;
                    }
        ...
        */
        case "show_details_item":
            ...
            $rows = DB::query("SELECT id, login, email, admin FROM ".prefixTable("users"));
            foreach ($rows as $record) {
                // Get auhtor
            ...
            $tags = "";
            $rows = DB::query("SELECT tag FROM ".prefixTable("tags")." WHERE item_id=%i", $post_id);
            foreach ($rows as $record) {
                if (empty($tags)) {
                    $tags = "<span style='' class='round-grey pointer tip' title='".langHdl('list_items_with_tag')."' onclick='searchItemsWithTags(\"".$record['tag']."\")'><i class='fa fa-tag fa-sm'></i>&nbsp;<span class=\"item_tag\">".$record['tag']."</span></span>";
                } else {
                    $tags .= "&nbsp;&nbsp;<span style='' class='round-grey pointer tip' title='".langHdl('list_items_with_tag')."' onclick='searchItemsWithTags(\"".$record['tag']."\")'><i class='fa fa-tag fa-sm'></i>&nbsp;<span class=\"item_tag\">".$record['tag']."</span></span>";
                }
            }
            ...
            // Check if actual USER can see this ITEM
                ...
                if (isset($SETTINGS['restricted_to_roles']) && $SETTINGS['restricted_to_roles'] === '1') {
                    ...
                    $rows = DB::query(
                        "SELECT t.title
                        FROM ".prefixTable("roles")."_title as t
                        INNER JOIN ".prefixTable("restriction_to_roles")." as r ON (t.id=r.role_id)
                        WHERE r.item_id = %i
                        ORDER BY t.title ASC",
                        $post_id
                ...
                if (empty($dataItem['url']) === false) {
                    $arrData['link'] = "&nbsp;<a href='".$dataItem['url']."' target='_blank'>&nbsp;<i class='fa fa-link tip' title='".langHdl('open_url_link')."'></i></a>";
                }
        ...
        */
        case "showDetailsStep2":
            ...
            // Check if actual USER can see this ITEM
                    ...
                    if (empty($reason[1]) === false || $record['action'] === "at_copy" || $record['action'] === "at_creation" || $record['action'] === "at_manual" || $record['action'] === "at_modification" || $record['action'] === "at_delete" || $record['action'] === "at_restored") {
                        if (trim($reason[0]) === "at_pw" && empty($reason[1]) === false) {
                            if (empty($historyOfPws)) {
                                $historyOfPws = langHdl('previous_pw')."\n".$reason[1];
                            } else {
                                $historyOfPws .= "\n".$reason[1];
                            }
                        }
                ...
                $files = $filesEdit = "";
                // launch query
                $rows = DB::query("SELECT id, name, file, extension FROM ".prefixTable("files")." WHERE id_item=%i", $post_id);
                foreach ($rows as $record) {
                    // get icon image depending on file format
                    $iconImage = fileFormatImage($record['extension']);
                    ...
                    // Prepare list of files for edit dialogbox
                    $filesEdit .= '<span id=\'span_edit_file_'.$record['id'].'\'><span class=\'fa fa-'.$iconImage.'\'></span>&nbsp;<span id=\'delete-edit-file_'.$record['id'].'\' class=\'fa fa-eraser tip file-eraser_icon\' style=\'cursor:pointer;\' onclick=\'delete_attached_file("'.$record['id'].'", "0")\' title=\''.langHdl('at_delete').'\'></span>&nbsp;'.$filename."</span><br />";
                }
                // display lists
        ...
        case 'lister_items_groupe':
            ...
            if (intval($start) === 0) {
                ...
                // check if this folder is visible
                    ...
                    DB::query("SELECT * FROM ".prefixTable("items")." WHERE inactif = %i", 0);
                    $counter = DB::count();
                    $uniqueLoadData['counter'] = $counter;
                }
        ...
        */
        case "get_refined_list_of_users":
            ...
            foreach ($rows as $record) {
                ...
                $rows2 = DB::query("SELECT id, login, fonction_id FROM ".prefixTable("users")." WHERE fonction_id LIKE '%".$record['role_id']."%'");
            ...
            foreach ($rows as $record) {
                ...
                if (empty($reason[1]) === false
                    ...
                    $history .= '<tr style="">'.
                        ...
                        '<td colspan="2" style="font-size:11px;"><i>'.langHdl('by').' '.$record['login'].' '.langHdl('at').' '.date($SETTINGS['date_format'].' '.$SETTINGS['time_format'], $record['date']).'</i></td></tr>'.
                        '<tr style="border-bottom:3px solid #C9C9C9;"><td style="width:100px;"><b>'.langHdl($record['action']).'</b></td>'.
                        '<td style="">'.(empty($record['raison']) === false && $record['action'] !== "at_creation" ? (count($reason) > 1 ? langHdl(trim($reason[0])).' : '.handleBackslash($reason[1]) : ($record['action'] === "at_manual" ? $reason[0] : langHdl(trim($reason[0])))) : '').'</td>'.
                        '</tr>'.
                        '<tr></tr>';
                }
            }
...
function fileFormatImage($ext)
{
    if (in_array($ext, TP_OFFICE_FILE_EXT)) {
        ...
        $image = "file-pdf-o";
    } elseif (in_array($ext, TP_IMAGES_FILE_EXT)) {
        $image = "file-image-o";
    } elseif ($ext === "txt") {
        $image = "file-text-o";
    } else {
