...
function performSaltKeyChange()
{
    // Confirm
    alertify
        .confirm(
            '<?php echo langHdl('warning'); ?>',
            '<?php echo langHdl('please_confirm_sk_change'); ?>',
            function() {
                // Show progress box
                $('#change-sk-progress').removeClass('hidden');
...
function confirmAttachmentsEncryption()
{
    // Confirm
    alertify
        .confirm(
            '<?php echo langHdl('warning'); ?>',
            '<?php echo langHdl('please_confirm_attachments_encryption_change'); ?>',
            function() {
                // Now start
                $.post(
                    'sources/admin.queries.php',
                    {
                        type    : 'admin_action_attachments_cryption',
                    ...
                    function(data) {
                        ...
                        if (data.error === true) {
                            $('#file-encryption-progress')
                                .html('<i class="fas fa-warning mr-2"></i>' + data.message);
                        } else {
                            // Show cog
                            alertify
                                .message('<i class="fas fa-cog fa-spin fa-2x"></i>', 0)
