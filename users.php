...
<div class="title ui-widget-content ui-corner-all">
    '.cleanText($LANG['admin_users']).'&nbsp;&nbsp;&nbsp;
    <button title="'.cleanText($LANG['new_user_title'], "css").'" onclick="OpenDialog(\'add_new_user\')" class="button" style="font-size:16px;">
        <i class="fa fa-plus"></i>
    </button>
    <button title="'.cleanText($LANG['share_user_rights'], "css").'" onclick="OpenDialog(\'share_rights_dialog\')" class="button" style="font-size:16px;">
...
<table id="t_users" class="hover" width="100%">
    <thead><tr>
        ...
        <th style="width:20px;" title="'.cleanText($LANG['god'], "css").'"><i class="fa fa-user-secret" style="font-size:14px;"></i></th>
        <th style="width:20px;" title="'.cleanText($LANG['gestionnaire'], "css").'"><i class="fa fa-child" style="font-size:14px;"></i></th>
        <th style="width:20px;" title="'.cleanText($LANG['read_only_account'], "css").'"><i class="fa fa-eye" style="font-size:14px;"></i></th>
        <th style="width:20px;" title="'.cleanText($LANG['can_manage_all_users'], "css").'"><i class="fa fa-group" style="font-size:14px;"></i></th>
        <th style="width:20px;" title="'.cleanText($LANG['can_create_root_folder'], "css").'"><i class="fa fa-code-fork" style="font-size:14px;"></i></th>
        <th style="width:20px;" title="'.cleanText($LANG['enable_personal_folder'], "css").'"><i class="fa fa-book" style="font-size:14px;"></i></th>
    </tr></thead>
    <tbody>
        <tr><td></td></tr>
    </tbody>
...
cleanText($LANG['change_user_functions_info']).'
...
cleanText($LANG['change_user_autgroups_info']).'
...
cleanText($LANG['change_user_forgroups_info']).'
...
<div id="change_user_adminby" style="display:none;">
    <div id="change_user_adminby_list" style="margin:20px 0 0 15px;">
        <select id="user_admin_by" class="input_text text ui-widget-content ui-corner-all">
            <option value="0">'.cleanText($LANG['administrators_only']).'</option>';
foreach ($rolesList as $fonction) {
    if ($_SESSION['is_admin'] || in_array($fonction['id'], $_SESSION['user_roles'])) {
        echo '
            <option value="'.$fonction['id'].'">'.cleanText($LANG['managers_of']).' "'.cleanText($fonction['title']).'"</option>';
    }
}
    echo '
        </select>
...
<div id="add_new_user" style="display:none;">
    ...
    <label for="new_name" class="label_cpm">'.cleanText($LANG['name']).'</label>
    ...
    <label for="new_lastname" class="label_cpm">'.cleanText($LANG['lastname']).'</label>
    ...
    <label for="new_email" class="label_cpm">'.cleanText($LANG['email']).'</label>
    ...
    <label for="new_is_admin_by" class="label_cpm">'.cleanText($LANG['is_administrated_by_role']).'</label>
...
echo '
    ...
    <label for="new_user_groups" class="form_label">'.cleanText($LANG['functions']).'</label>
...
foreach ($rows as $record) {
    ...
    <label for="new_user_auth_folders" class="form_label" style="margin-top:3px;">'.cleanText($LANG['authorized_groups']).'</label>
    <select name="new_user_auth_folders" id="new_user_auth_folders" multiple="multiple">
        '.$foldersList.'
    </select>

    <label for="new_user_forbid_folders" class="form_label" style="margin-top:3px;">'.cleanText($LANG['forbidden_groups']).'</label>
    <select name="new_user_forbid_folders" id="new_user_forbid_folders" multiple="multiple">
        '.$foldersList.'
    </select>

    <div style="text-align:left;margin-top:5px;">
        <label style="">'.cleanText($LANG['admin_misc_title']).'</label>
        <div style="margin-top:5px;">
            <table border="0">
            <tr>
                <td>', $_SESSION['user_admin'] === "1" ? '
                    <input type="checkbox" id="new_admin" style="margin-bottom:3px;" />
                    <label for="new_admin">'.cleanText($LANG['is_admin']).'</label>

                    <input type="checkbox" id="new_super_manager" style="margin-bottom:3px;" />
                    <label for="new_super_manager">'.cleanText($LANG['is_super_manager']).'</label>

                    <input type="checkbox" id="new_manager" style="margin-bottom:3px;" />
                    <label for="new_manager">'.cleanText($LANG['is_manager']).'</label>
                    ' : '', '

                    <input type="checkbox" id="new_read_only" style="margin-bottom:3px;" />
                    <label for="new_read_only">'.cleanText($LANG['is_read_only']).'</label>
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                <td>
                    <div id="auto_create_folder_role"  style="visibility:hidden;">
                        ...
                        <label for="new_folder_role_domain">'.cleanText($LANG['auto_create_folder_role']).'&nbsp;&quot;<span id="auto_create_folder_role_span"></span>&quot;</label>
...
cleanText($LANG['give_new_pw']).'
    <div style="font-weight:bold;text-align:center;color:#FF8000;display:inline;" id="change_user_pw_show_login"></div>
    <div style="margin-top:20px;">
        <table>
            <tr>
                <td style="width:120px;"><label for="change_user_pw_newpw">'.cleanText($LANG['index_new_pw']).'</label>&nbsp;</td>
                <td><input type="password" size="30" id="change_user_pw_newpw" /></td>
            </tr>
            <tr>
                <td><label for="change_user_pw_newpw_confirm">'.cleanText($LANG['index_change_pw_confirmation']).'</label>&nbsp;</td>
