...
$var['hidden_asterisk'] = '<i class="fa fa-asterisk"></i>&nbsp;<i class="fa fa-asterisk"></i>&nbsp;<i class="fa fa-asterisk"></i>&nbsp;<i class="fa fa-asterisk"></i>&nbsp;<i class="fa fa-asterisk"></i>';
...
function ListerItems(groupe_id, restricted, start, stop_listing_current_folder)
{
  ...
  // show list of items if not visible
  showItemElement("items_list_div");
  ...
  if (groupe_id != undefined) {
    ...
    if ($("#reading_folder").length === 0) {
      $("#items_path_var").append('<div class="alert alert-warning" role="alert" style="margin-bottom:10px;" id="reading_folder"><i class="fa fa-folder-open-o"></i>&nbsp;<?php echo $LANG['opening_folder'];?></div>');
    }
    ...
    request = $.post("sources/items.queries.php",
      ...
      function(data) {
        ...
        if (data.arborescence != undefined) {
          $("#items_path_var").html('<ol class="breadcrumb">' + data.arborescence + '</ol>');
          /*
        ...
        if (data.array_items == "" && data.items_count == "0") {
          $("#items_list1").html('<div style="text-align:center;margin-top:30px;"><b><i class="fa fa-info-circle"></i>&nbsp;<?php echo addslashes($LANG['no_item_to_display']);?></b></div>');
        }
        ...
        if (data.error == "is_pf_but_no_saltkey") {
          ...
          $("#item_details_ok, #items_list1").show();
          ...
          if (data.list_to_be_continued == "yes") {
            ...
            $("#items_list1").append(data.items_html);
          ...
          */
          ...
          proceed_list_update(0, data.items_count);
        } else {
          $("#recherche_group_pf").val(data.saltkey_is_required);
          //Display items
          $("#item_details_no_personal_saltkey, #item_details_nok").hide();
      ...
      $.post(
        ...
        function(data) {
          ...
          if (data.error == "item_exists") {
            ...
            $("#items_list1").append(data.new_entry);
        ...
        function(data) {
          ...
          if (data.error == "ERR_JSON_FORMAT") {
            ...
            if (data.reload_page == "1") {
              ...
              //increment / decrement number of items in folders
              $("#itcount_"+$('#hid_cat').val()).text(Math.floor($("#itcount_"+$('#hid_cat').val()).text())-1);
              $("#itcount_"+$('#item-folder-input').val()).text(Math.floor($("#itcount_"+$('#item-folder-input').val()).text())+1);
            }
...
function AfficherDetailsItem(id, salt_key_required, expired_item, restricted, display, open_edit, reload, id_tree)
{
  ...
  echo 0;
    ...
    //} else if ($("#request_lastItem").val() == id && reload != 1) {console.log("ici2");
    //  $("#request_ongoing").val("");
    //  LoadingPage();
    //  return;
    } else {
      ...
      $.post(
        ...
        function(data) {
          ...
          // show buttons for previous and next items
          if ($("#" + id).prev("tr").attr("id") === undefined) {
            $("#previous_item").val("");
            $("#but_previous_item").attr('disabled', 'disabled');
          } else {
            $("#previous_item").val($("#" + id).prev("tr").attr("id"));
            $("#but_previous_item").attr('disabled', false);
          }
          if ($("#" + id).next("tr").attr("id") === undefined) {
            $("#next_item").val("");
            $("#but_next_item").attr('disabled', 'disabled');
          } else {
            $("#next_item").val($("#" + id).next("tr").attr("id"));
            $("#but_next_item").attr('disabled', false);
          }
          ...
          if (data.show_details == "1" && data.show_detail_option != "2") {
            ...
            if (data.to_be_deleted != 0 && data.to_be_deleted != null && data.to_be_deleted != "not_enabled") {
              $('#item_extra_info')
                .html("<span class='label label-primary item_field tip'><i class='fa fa-bell-o mi-red'></i></span>&nbsp;")
            ...
            if (data.pw != "") {
              ...
              clipboard_pw.on('success', function(e) {
                displayMessage("<?php echo $LANG['pw_copied_clipboard'];?>");
            ...
            if (data.login != "") {
              ...
              clipboard_login.on('success', function(e) {
                displayMessage("<?php echo $LANG['login_copied_clipboard'];?>");
                e.clearSelection();
              });
              $("#button_quick_login_copy").show();
            }
            ...
            if (data.url != "") {
              ...
              clipboard_url.on('success', function(e) {
                displayMessage("<?php echo $LANG['url_copied_clipboard'];?>");
                e.clearSelection();
              });
            }
            ...
            clipboard_link.on('success', function(e) {
              displayMessage("<?php echo $LANG['url_copied'];?>");
              e.clearSelection();
            });
  ...
  showItemElement("item_detail_div");
  }
}
...
{
  if (itemTitle != "") {
    $("#item-label-input").removeClass('has-danger');
      ...
      $.post(
        ...
        function(data) {
          if (data[0].duplicate != "1") {
            ...
            $("#label").focus();
            $("#item-label-input").addClass('has-danger');
            displayMessage("<?php echo $LANG['duplicate_title_in_same_folder'];?>").show();
          }
        }
      );
    } else {
...
function quickShowItem(elem) {
  var item_id = $("#"+elem).val();
  if (item_id !== "") {
    var param = $("#" + item_id).attr("data-param").split(',');
    AfficherDetailsItem(item_id, param[0], param[1], param[2], "", "", "", "");
  }
}


function prepare_item_edition(restricted_to_roles) {
  // is user read only and it is not a personal folder
  if (
    ($('#recherche_group_pf').val() == 0 && $("#user_is_read_only").length && $("#user_is_read_only").val() == "1") ||
    $("#access_level").val() == "NE" || $("#access_level").val() == "NDNE"
  ...
  // Show WYGIWYG editor
  $("#item-description-input").summernote({
    lang: "<?php echo strtolower($_SESSION['user_language_code']).'-'.strtoupper($_SESSION['user_language_code']);?>",
    toolbar: [
  ...
  $('#item-url-input').val($('#hid_url').val());
  $('#item-folder-input').val($('#id_categorie').val());
  $('#item-folder-input').select2({
    language: "<?php echo $_SESSION['user_language_code'];?>"
  });

  if ($('#item-restrictedto-input').val() != undefined) {
    $('#item-restrictedto-input').val($('#hid_restricted_to').val());
  }
  if ($('#item-restrictedtoroles-input').val() != undefined) {
    $('#item-restrictedtoroles-input').val($('#hid_restricted_to_roles').val());
  }
  $('#edit_tags').val($('#hid_tags').val());
  if ($('#hid_anyone_can_modify').val() == "1") {
    $('#edit_anyone_can_modify').attr("checked","checked");
  ...
  // disable folder selection if PF
  if ($('#recherche_group_pf').val() == "1") {
    $("#item-folder-input").prop("disabled", true);
  } else {
...
function saveItemDefinition() {
  // first check that form is correctly filled in
  if (!IsValidEmail($("#item-email-input").val()) && $("#item-email-input").val() !== "") {
    $("#item-email-input").addClass('has-warning');
    $("#item-definition-conform").val("0");
  }
  if (!validateURL($("#item-url-input").val()) && $("#item-url-input").val() !== "") {
    $("#item-url-input").addClass('has-warning');
    $("#item-definition-conform").val("0");
  }

  if ($("#item-definition-conform").val() === "0") {
    displayMessage("Item definition is not correct!", "error", 3000);
    return false;
  }
...
$(function() {
  ...
  $('#edit_item_navigator a').click(function (e) {
    e.preventDefault();
    $(this).tab('show');
  });
  ...
  $("body").on('change','.cb-multi-item', function () {
    if($('input.cb-multi-item:checked').length > 0) {
      $("#button-items-multi-selection").show();
    } else {
      $("#button-items-multi-selection").hide();
    }
  });
 ...
 */
function proceed_list_update(stop_proceeding, items_count)
{
  stop_proceeding = stop_proceeding || "";

  // prepare navigation buttons
  var nb_pages = Math.round(items_count / 50);
  if (nb_pages > 1){
    $('#items_pages_navigation').bootpag({
      total: nb_pages,
      ...
      var new_query_start = ((num-1) * 50);
      $("#current_page_number").val(num);
      ListerItems($("#hid_cat").val(),'', parseInt(new_query_start));
    });
  } else {
    $('#items_pages_navigation').html("");
  }

  /*
  if (nb_pages > 3) {
    var navigator = '<li class="page-item disabled">' +
      '<a class="page-link" href="#" tabindex="-1" data-page-number="previous">&laquo;</a>' +
      '</li>';
    var page_min = 3,
      page_max = nb_pages - 1;
    ...
    for (x = 1; x <= nb_pages ; x ++) {
      if (x === page_min) {
        navigator += '<li class="page-item disabled"><a class="page-link" href="#" data-page-number="">...</a></li>';
      } else if (x > page_min && x < page_max) {

      } else if (x >= page_max) {
        navigator += '<li class="page-item"><a class="page-link" href="#" data-page-number="' + x + '">' + x + '</a></li>';
      } else {
        navigator += '<li class="page-item"><a class="page-link" href="#" data-page-number="' + x + '">' + x + '</a></li>';
      }
    }

    navigator += '<li class="page-item">' +
      '<a class="page-link" href="#" data-page-number="next">&raquo;</a>' +
      '</li>';

    $("#items_pages_navigation").html(navigator);
  } else if (nb_pages === "2") {
    $("#items_pages_navigation").html(
      '<li class="page-item"><a class="page-link" href="#" data-page-number="1">1</a></li>'+
      '<li class="page-item"><a class="page-link" href="#" data-page-number="2">2</a></li>'
    );
  } else if (nb_pages === "3") {
    $("#items_pages_navigation").html(
      '<li class="page-item"><a class="page-link" href="#" data-page-number="1">1</a></li>'+
      '<li class="page-item"><a class="page-link" href="#" data-page-number="2">2</a></li>'+
      '<li class="page-item"><a class="page-link" href="#" data-page-number="3">3</a></li>'
    );
  }

  $(".page-link").click(function() {
    var new_page_to_show = $(this).attr('data-page-number');

    // manage next/prev cases
    if (new_page_to_show === "previous") {
      new_page_to_show = $("#current_page_number").val()--;
    } else if (new_page_to_show === "next") {
      new_page_to_show = $("#current_page_number").val()++;
    }

    // store new page
    $("#current_page_number").val(new_page_to_show);

    // calculate next start
    var new_query_start = ((new_page_to_show-1) * 50);
      alert(new_page_to_show+" -- "+new_query_start);
    ListerItems($("#hid_cat").val(),'', parseInt(new_query_start));
  });
  */
  ...
  $.post(
    ...
    function(data) {
      ...
      if (data.error == "") {
        ...
        clipboard.on('success', function(e) {
          displayMessage("<?php echo $LANG['link_is_copied'];?>");
          e.clearSelection();
        });
